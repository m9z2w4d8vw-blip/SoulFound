# SoulFound

A download-only Soulseek client for iPhone. Search and download files from the Soulseek network directly to your phone — no computer needed.

## I know this app is shit, SO I DONT NEED YOU TELL ME THAT 😠😠😠😠😠😠

## Install
Download ipa from releases and install however u want as long as ur on ios 17.0+ (trollstore, altstore, sideloadly, etc.) 

## Hi :3
Fuck you. ermmmm i vibecoded this entire thing so i really dont know shit about bugs and shit so i really cant help but i do know how to use claude so ill try to solve any issues. but to reiterate i vibecoded this ENTIRE thing - i have no idea how to actually code in swift whatsoever. anyways, fuck you. 

## Protocol reference (claude generated ts)

- [Nicotine+ Soulseek Protocol Docs](https://nicotine-plus.org/doc/SLSKPROTOCOL.html)
- [aioslsk flow documentation](https://aioslsk.readthedocs.io/en/latest/SOULSEEK.html)
